#!/usr/bin/env bash
# One-line installer for MergN — clones the repo to a fixed location and puts
# the `mergn` command on your PATH, so you can run `mergn run` / `mergn update`
# from anywhere.
#
...
ln_cmd() { if [ -w "$BIN_DIR" ]; then ln -sf "$1" "$2"; else sudo ln -sf "$1" "$2"; fi; }
...
echo "✓ Installed — 'mergn' is now on your PATH ($BIN_DIR/mergn), repo at $DEST"
echo
echo "Next:"

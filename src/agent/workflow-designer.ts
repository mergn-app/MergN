...
const PLAN_SYSTEM = [
  ...
  "CONDITIONAL ACTIONS: there is no branch node — every step runs. When an action must only happen in some cases (refund ONLY when approved, create a record ONLY when not a duplicate, alert ONLY when risk is high), have a deciding step output a flag (approval_status, is_duplicate, is_high_risk) and add a dep so that flag flows into EACH gated action step (which then guards itself on it). Always route the decision flag to every action it controls.",
...
const BODY_SYSTEM = [
  ...
  "CONDITIONAL ACTIONS: the engine runs EVERY step — there is no graph-level branch. So if an action must only happen in some cases (refund only when approved, create a record only when NOT a duplicate, alert only when risk is high), the deciding step must output a clear flag (e.g. approval_status, is_duplicate, is_high_risk) and EACH dependent action step must read that flag and GUARD ITSELF at the top: `if (input.approval_status !== 'approved') return { <its declared outputs set to null>, skipped: true };` before doing the side effect. Always return the step's normal output shape (nulls) when skipped so downstream steps don't break. Never perform an irreversible side effect (a refund, a charge, creating a record, sending a message) without checking the gating flag the decision step produced.",

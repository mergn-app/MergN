...
const SYSTEM = [
  ...
  "FILE UPLOADS: if the service can send/receive file attachments (Discord/Slack/Telegram messages with files, email attachments, object storage), include a dedicated method that takes the file and uploads it via multipart/form-data — NOT JSON. A file is passed in as an object { name, mime, base64 }. Use the runtime globals Buffer, Blob and FormData: const blob = new Blob([Buffer.from(file.base64, 'base64')], { type: file.mime }); const fd = new FormData(); fd.append('file', blob, file.name); then fetch(url, { method:'POST', headers:{ Authorization: ... }, body: fd }). Do NOT set the Content-Type header for multipart (fetch sets the boundary). Discord example — sendFile(channelId, file, content): const fd = new FormData(); fd.append('payload_json', JSON.stringify({ content: content ?? '' })); fd.append('files[0]', new Blob([Buffer.from(file.base64,'base64')], { type: file.mime }), file.name); await fetch(`https://discord.com/api/channels/${channelId}/messages`, { method:'POST', headers:{ Authorization: `Bot ${cred.apiKey}` }, body: fd }).",
...
const REPAIR_SYSTEM = [
  ...
  "Fix the clientSource so the call succeeds. Keep the SAME id, name, authEnv, sandbox, credential.fields, and keep all EXISTING method names/signatures — only fix their implementation. EXCEPTION: if the error is that the failing step calls a method that does not exist yet (e.g. 'ctx.connections.x.sendFile is not a function'), ADD that method to the factory, matching the call site's name and arguments.",

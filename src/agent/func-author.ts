...
const SYSTEM = [
  ...
  "- SETTINGS vs DATA: give role 'config' to a fixed per-step SETTING the user configures once — a destination/location, not flowing data (a spreadsheet id, sheet/column name, a Slack/Discord channel id, a Trello board/list id, an Asana project id, a Mailchimp audience id, an Airtable table name, an API/webhook URL, a threshold). Give role 'input' to data that flows from the trigger or an upstream step (an email, amount, name, score, the parsed item). Config is kept per-step, so two steps can each have their own spreadsheet id without colliding.",
  "- Return ONLY outputs that a later step or the final action consumes, plus the step's primary result. Never echo an input straight back as an output. If the step processes a LIST, return the list as one output — do not emit per-item scalar fields as outputs.",
  "- CONDITIONAL ACTIONS: every step runs (no branch node). If this step is an action that must only happen in some cases (refund only when approved, create only when not a duplicate), read the gating flag from input (e.g. input.approval_status) and GUARD at the top: `if (input.approval_status !== 'approved') return { <declared outputs as null>, skipped: true };` before the side effect. Never do an irreversible side effect without checking the flag.",
